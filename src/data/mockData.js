...
export { sampleCbtQuestionBank } from './cbtQuestionBankData.js';
...
// =============================================================================
// REACTIVE TEST SERVICE: Canonical Adapter delegating to cbtTestService
// Unifies the prototype onto ONE canonical assessment engine
// =============================================================================

export const testService = {
  getTests: () => {
    return cbtTestService.getAllTests('all');
  },

  getAllTests: (trackFilter = 'all') => {
    return cbtTestService.getAllTests(trackFilter);
  },

  getTestById: (id) => {
    return cbtTestService.getTestById(id) || cbtTestService.getAllTests('all')[0];
  },

  saveTest: (newTest) => {
    const existing = cbtTestService.getTestById(newTest.id);
    if (existing) {
      return cbtTestService.updateTest(newTest.id, newTest);
    }
    return cbtTestService.createTest(newTest);
  },
  ...
  addTest: (testData) => {
    ...
    const durationMinutes = typeof testData.duration === 'string' 
      ? parseInt(testData.duration, 10) || 45 
      : (testData.durationMinutes || 45);

    const date = testData.dateTime ? testData.dateTime.split('@')[0]?.trim() : (testData.date || 'Upcoming');
    const time = testData.dateTime ? testData.dateTime.split('@')[1]?.trim() : (testData.time || '18:00 IST');

    return cbtTestService.createTest({
      name: testData.name,
      title: testData.name,
      courseId: testData.courseId || 'neet-pg',
      examTrack: testData.courseId || 'neet-pg',
      ...
      batchTier: testData.batch || 'All Enrolled Students',
      date,
      time,
      durationMinutes,
      totalMarks: testData.totalMarks || 100,
      questionCount: questionList.length > 0 ? questionList.length : (testData.totalQuestions || 20),
      questions: questionList,
      questionIds: testData.questionIds || [],
      pattern: testData.pattern || 'Clinical Vignettes (Single Best Response)'
    });
  },

  updateTestQuestions: (testId, questions) => {
    return cbtTestService.updateTestQuestions(testId, questions);
  },

  getQuestionsForTest: (testId) => {
    return cbtTestService.getQuestionsForTest(testId);
  },

  subscribe: (callback) => {
    const handler = () => {
      callback(cbtTestService.getAllTests('all'));
    };
    window.addEventListener('medprep-tests-updated', handler);
    window.addEventListener('medprep-cbt-tests-updated', handler);
    return () => {
      window.removeEventListener('medprep-tests-updated', handler);
  ...
  getCohortResults: (testId) => {
    return cbtTestService.getCohortResults(testId);
  },

  submitStudentAttempt: (testId, attemptData) => {
    // Submit via canonical evaluation engine
    const submitted = cbtTestService.submitAttempt(testId, 'normal');
    return { 
      test: cbtTestService.getTestById(testId), 
      cohortResults: cbtTestService.getCohortResults(testId),
      submitted 
    };
  }
};
