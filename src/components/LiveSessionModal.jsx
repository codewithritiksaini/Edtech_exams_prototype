...
export default function LiveSessionModal({ isOpen, onClose, session }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col h-screen max-h-screen h-[100dvh] max-h-[100dvh] w-screen overflow-hidden text-white animate-in fade-in duration-200">
      {/* Top Header - Fixed height */}
      <div className="h-14 sm:h-16 px-4 sm:px-6 bg-slate-950/95 border-b border-slate-800 flex items-center justify-between shrink-0 z-20">
        <div className="flex items-center gap-3 min-w-0">
          <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-600/90 text-white text-[11px] sm:text-xs font-bold uppercase tracking-wider shadow-sm shrink-0">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-300 bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-700/60">
            <Users className="w-3.5 h-3.5 text-brand-400" />
            <span>{(session.attendeesCount || 120) + 1} Doctors Online</span>
          </div>
          <button
            onClick={onClose}
      ...
      {/* Center Content: Video Stream + Interactive Chat - Exactly fits remaining viewport height */}
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 overflow-hidden w-full">
        
        {/* Main Video Stream */}
        <div className="lg:col-span-8 xl:col-span-8 2xl:col-span-9 bg-slate-950 flex flex-col justify-between p-3 sm:p-4 h-full min-h-0 overflow-hidden">
          
          {/* Faculty Video Simulation Screen */}
          <div className="relative w-full flex-1 min-h-0 rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 flex flex-col justify-between p-4 sm:p-5 shadow-2xl">
            <img
            ...
            <div className="relative z-10 flex items-center justify-between shrink-0">
              <div className="bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-white">{session.faculty || 'Dr. Siddharth V.'}</span>
              </div>
            ...
            <div className="relative z-10 max-w-md mx-auto text-center bg-slate-950/85 p-4 sm:p-5 rounded-2xl border border-slate-800/90 backdrop-blur-sm space-y-2 shrink-0 my-auto">
            ...
            <div className="relative z-10 flex items-center justify-between bg-black/70 backdrop-blur-md p-3 rounded-xl border border-white/10 shrink-0">
        ...
        <div className="lg:col-span-4 xl:col-span-4 2xl:col-span-3 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-800 flex flex-col h-full min-h-0 overflow-hidden">
          ...
          <div className="p-3 sm:p-4 space-y-2.5 overflow-y-auto flex-1 min-h-0 text-xs">
          ...
          {/* Message Input Box */}
          <form onSubmit={handleSendMessage} className="p-3 border-t border-slate-800 bg-slate-950/70 flex items-center gap-2 shrink-0">
            <input
              type="text"
              value={inputText}
