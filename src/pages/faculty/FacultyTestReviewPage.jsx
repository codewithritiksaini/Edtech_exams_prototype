...
export default function FacultyTestReviewPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const unsubscribe = cbtTestService.subscribe((allTests) => {
  ...
  const readiness = useMemo(() => {
    if (!test) return null;
    return testReadinessService.validateTestForPublish(test, exam, { role: 'faculty' });
  }, [test, exam]);

  const isAlreadyPublished = testPublishService.isTestPublished(test);
  ...
  const handleSaveWindow = (windowConfig) => {
    try {
      ...
      if (windowConfig.enabled && windowConfig.startAt) {
        const startDate = new Date(windowConfig.startAt);
        updated.date = startDate.toLocaleDateString();
        updated.time = `${startDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} IST`;
        updated.formattedWindow = `${updated.date} • ${updated.time}`;
      }
      const idx = cbtTestService.tests.findIndex(t => t.id === id);
      if (idx !== -1) {
        cbtTestService.tests[idx] = updated;
  ...
  const handleConfirmPublish = () => {
    try {
      ...
      const published = testPublishService.publishFacultyTest(id, {
        testWindow: test.testWindow,
        publishedBy: facultyProfile?.name || 'Faculty'
      }, facultyProfile);
      setTest(published);
      setPublishModalOpen(false);
      setIsPublishing(false);
      setSuccessToast(`🎉 Assessment "${published.name}" has been published to student schedule!`);
    } catch (err) {
      setIsPublishing(false);
      alert(err.message);
    }
    ...
    return (
      <div className="p-8 max-w-5xl mx-auto space-y-4">
        <div className="p-6 bg-rose-50 border border-rose-200 rounded-2xl text-rose-900 text-sm">
          Faculty test with ID "{id}" was not found.
        </div>
        <Link to="/faculty/tests" className="text-xs text-indigo-600 font-bold hover:underline">
          ← Back to Tests
        </Link>
      </div>
    );
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-slate-500">
                <span className="flex items-center gap-1.5">
                  ...
                  Exam Curriculum: <strong className="text-slate-800">{exam?.name || test.examId}</strong>
                </span>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
          ...
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border flex items-center gap-1.5 self-start sm:self-center ${
            ...
            {isAlreadyPublished ? (
              <>
                <CheckCircle2 className="w-3.5 h-3.5" /> Scheduled / Published
              </>
            ) : readiness?.ready ? (
              <>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-5 space-y-3.5 text-xs">
              ...
              {test.testWindow?.enabled ? (
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-slate-500 italic text-[11px]">
                  {test.formattedWindow || 'Upcoming • Continuous Access'}
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 space-y-2.5 text-xs text-slate-600">
              <h4 className="font-bold text-slate-800 flex items-center gap-1.5">
                ...
                Faculty Lifecycle Sealing
              </h4>
              <p className="leading-relaxed text-[11px] text-slate-500">
                When published, this assessment transitions to student upcoming queues and is locked to maintain examination validity.
              </p>
            </div>
          </div>
        </div>
