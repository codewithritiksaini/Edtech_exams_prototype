...
export default function DayContentView() {
  ...
  // When dayId changes, make sure activeTab is an allowed tab and reset states
  useEffect(() => {
  ...
  const activePdf = (currentDayData.pdfList && currentDayData.pdfList[selectedPdfIndex]) || currentDayData.pdf || {
    ...
    pages: 12,
    size: '3.4 MB',
    author: 'Dr. Anita Sharma, MD (AIIMS)'
  };
        ...
        <main className="flex-grow p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto w-full space-y-8">
                ...
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                  ...
                  <button
                    onClick={() => {
                      setIsCompleted(true);
                      setTimeout(() => {
                        navigate(`/dashboard?completedDay=${currentNum}`);
                      }, 900);
                    }}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-2 overflow-x-auto">
              ...
              {/* Tab 6: Timed CBT Test (Phase 5 Dynamic LMS Integration) */}
              {(currentDayData.hasTest || currentDayData.activeTabs.includes('test')) && (
                <button
                ...
                >
                  ...
                  <span className="text-[10px] px-1.5 py-0.2 rounded bg-purple-200 text-purple-900 font-black">EXAM</span>
                </button>
              )}
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm min-h-[460px]">
              ...
              {activeTab === 'notes' && (
                <div className="space-y-6 animate-in fade-in">
                  ...
                  {/* Multi-PDF Document Switcher (Phase 5 Aggregated Assets) */}
                  {currentDayData.pdfList && currentDayData.pdfList.length > 1 && (
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
                      <span className="text-xs font-bold text-slate-500 uppercase tracking-wider shrink-0 flex items-center gap-1">
                        ...
                        Documents ({currentDayData.pdfList.length}):
                      </span>
                      {currentDayData.pdfList.map((doc, idx) => (
                        <button
                          key={doc.id || idx}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-slate-900 leading-snug">
                          {activePdf?.title || 'High-Yield Clinical Notes'}
                        </h3>
                        <p className="text-xs text-slate-500">
                          {activePdf?.fileName} • {activePdf?.pages} Pages • {activePdf?.size}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => alert(`Downloading ${activePdf?.fileName}...`)}
                        className="px-3.5 py-2 rounded-xl text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-100 transition-colors flex items-center gap-1.5 shadow-2xs cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl text-xs font-bold text-white bg-brand-600 hover:bg-brand-700 transition-colors flex items-center gap-1.5 shadow-sm cursor-pointer"
                      >
                  ...
                  <div className="border border-slate-200 rounded-2xl p-6 sm:p-8 bg-slate-50/50 space-y-6">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                        Page 1 of {activePdf?.pages || 12} • Document Preview
                      </span>
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5 text-slate-800 text-xs sm:text-sm leading-relaxed">
                      <div className="border-b border-slate-100 pb-3">
                        <h4 className="text-base font-extrabold text-slate-900">
                          {activePdf?.title || 'Diagnostic Algorithm: Rapid Clinical Approach'}
                        </h4>
                        <p className="text-xs text-slate-500 mt-0.5">
                          Authored by {activePdf?.author || 'Academic Board'}
                        </p>
                      </div>
              ...
              {/* --------------------------------------------------------------------- */}
              {/* TAB 6: TIMED CBT MOCK TEST                                            */}
              {/* --------------------------------------------------------------------- */}
              {activeTab === 'test' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-gradient-to-br from-purple-50 via-white to-indigo-50 text-slate-900 rounded-3xl p-8 border border-purple-200 shadow-sm space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-600 text-white text-xs font-bold shadow-xs">
                        <Award className="w-3.5 h-3.5 text-white" />
                        <span>Scheduled Clinical CBT Mock Test</span>
                      </div>
                      <span className="text-xs font-mono font-bold text-purple-700 bg-purple-100 border border-purple-200 px-3 py-1 rounded-xl">
                        Day {currentDayData.dayNumber} Assessment
                      </span>
                    </div>

                    <div className="space-y-2">
                      <h3 className="text-xl sm:text-2xl font-black text-slate-900">
                        {currentDayData.subjectName ? `${currentDayData.subjectName} — Daily Benchmark Examination` : 'Clinical Competency CBT Test'}
                      </h3>
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed max-w-2xl">
                        Simulated computer-based test adhering to official exam format. Features single-best answer clinical vignettes, negative marking simulation, and instant percentile analytics.
                      </p>
                    </div>

                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-2">
                      <div className="p-3 bg-white rounded-2xl border border-purple-100 shadow-2xs text-center">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Questions</span>
                        <span className="text-base font-black text-purple-700">25 MCQs</span>
                      </div>
                      <div className="p-3 bg-white rounded-2xl border border-purple-100 shadow-2xs text-center">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Duration</span>
                        <span className="text-base font-black text-purple-700">30 Mins</span>
                      </div>
                      <div className="p-3 bg-white rounded-2xl border border-purple-100 shadow-2xs text-center">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Marking</span>
                        <span className="text-base font-black text-purple-700">+4 / -1</span>
                      </div>
                      <div className="p-3 bg-white rounded-2xl border border-purple-100 shadow-2xs text-center">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Passing Cutoff</span>
                        <span className="text-base font-black text-emerald-600">65%</span>
                      </div>
                    </div>

                    <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-purple-100">
                      <span className="text-xs text-slate-500 font-medium">
                        ⏱️ Timed simulation begins immediately upon launch.
                      </span>
                      <button
                        onClick={() => navigate('/test/test-cardio-01')}
                        className="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs sm:text-sm rounded-xl shadow-md shadow-purple-600/20 transition-all flex items-center gap-2 cursor-pointer"
                      >
                        <Play className="w-4 h-4 fill-current" />
                        <span>Launch Computer-Based Test</span>
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="w-screen max-w-4xl bg-white shadow-2xl border-l border-slate-200 flex flex-col h-full animate-in slide-in-from-right duration-300">
              ...
              <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between shrink-0 sticky top-0 z-10">
                <div>
                  ...
                  <h4 className="text-sm font-bold text-white mt-0.5">{activePdf?.title || 'Clinical Study Notes'}</h4>
                </div>
                <button 
                  onClick={() => setFullscreenPdfOpen(false)} 
              ...
              <div className="px-6 py-3.5 bg-slate-100 border-t border-slate-200 flex justify-between items-center text-xs shrink-0 sticky bottom-0 z-10">
                <span className="font-semibold text-slate-600">Page 1 of {activePdf?.pages || 12}</span>
                <button 
                  onClick={() => setFullscreenPdfOpen(false)} 
