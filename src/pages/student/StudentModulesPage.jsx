...
export default function StudentModulesPage() {
  ...
  const [modules, setModules] = useState(() => curriculumService.getModulesBySubject(subjectId));
  const [searchQuery, setSearchQuery] = useState('');

  // Module completion simulation
  const completionMap = {
    'mod-1': { completed: true, score: '94% Recall', status: 'Mastered' },
    'mod-2': { completed: true, score: '88% Recall', status: 'Mastered' },
    'mod-3': { completed: false, score: 'In Progress', status: 'Active Today' },
  };
  ...
  useEffect(() => {
    ...
    const chapList = curriculumService.getModulesBySubject(subjectId);
    setModules(chapList || []);
  }, [examId, subjectId]);

  const filteredModules = modules.filter(c => 
    ...
    <div className="space-y-6 animate-in fade-in max-w-6xl">
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight mt-1">
              Unit Modules & Clinical Syllabus
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 max-w-2xl mt-1">
              Follow the recommended learning sequence. Each module contains focused clinical lectures, video lectures, and active recall flashcards.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredModules.map((chap, idx) => {
          const lectures = curriculumService.getLecturesByModule(chap.id) || [];
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 pt-1 text-xs font-semibold text-slate-600">
                  <span className="px-2.5 py-1 rounded-xl bg-slate-100 border border-slate-200/60 flex items-center gap-1">
                    ...
                    <span>{lectures.length || chap.lecturesCount || 3} Study Lectures</span>
                  </span>
                  <span className="px-2.5 py-1 rounded-xl bg-slate-100 border border-slate-200/60 flex items-center gap-1">
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] text-slate-400 font-medium">
                  Hierarchy Level 4 ➡️ Lectures
                </span>
                <Link
                  to={`/student/courses/${examId}/subjects/${subjectId}/modules/${chap.id}/lectures`}
